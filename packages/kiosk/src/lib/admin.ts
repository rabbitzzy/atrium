 ...
 * screen they can reach. It is a deployment step, like the printer name.
 *
 * That it is a shared secret at all is the temporary part. `api/_lib/admin.ts`
 * calls itself a guard rather than an authentication system, and real per-user
 * access arrives with the teacher dashboard, which needs identity instead of a
